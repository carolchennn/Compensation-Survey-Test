...
Last changed: 2014.01.17 - Acronym|PCL */
/************************ ADDITIONAL FEATURES ************************
	 Plugins
*/
var s_account;
...
if(!s_account)
{
	var s_account="almglobal" //default s_account if nothing is passed from the page
...
{
	s.linkInternalFilters = 'javascript:,law.com,localhost'; //default s.linkInternalFilters if nothing is passed from the page
}
else
{
...
s.visitorNamespace="alm"
s.trackingServer="bc.law.com"
s.trackingServerSecure="b.law.com"
s.visitorMigrationKey="4D013A4B"
...
s.linkTrackVars="prop5,prop6,prop7"
...
/* Initialize jQuery if not already loaded */
if(!window.jQuery){
	document.write('<scr' + 'ipt language="javascript" type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js"></scr' + 'ipt>');
}

/* Small function to shorthand some array objects */
function ary(a) {
	var o = {};
...
/* Share comment - event22 */
function scComment(){
   var s = s_gi(s_account);
   s.linkTrackVars = "events,prop11,eVar11";
   s.prop11 = s.prop11;
   s.eVar11 = s.prop11;
...
/* Call slidshow function */
$(document).ready(function(){
	$("button.next").click(function(){
		scSlideshow("slideshow:next");
	});
});
$(document).ready(function(){
	$("button.prev").click(function(){
		scSlideshow("slideshow:prev");
	});
});

/* Slideshow Function - s.prop38 */
function scSlideshow(slideshowType){
   var s = s_gi(s_account);
   //s.linkTrackVars = "prop38,eVar38";
   //s.tl(this,'o','slideshow');
   var s_code=s.t();if(s_code)document.write(s_code)
}

/* Set Social OnClick values */
$(document).ready(function(){
	$(".share-linkedin").click(function(){
		scShare("share-linkedin");
	});
	$(".share-facebook").click(function(){
		scShare("share-facebook");
	});
	$(".share-twitter").click(function(){
		scShare("share-twitter");
	});
	$(".share-googleplus").click(function(){
		scShare("share-googleplus");
	});
...
/* Share Function - s.prop29, event6 */
function scShare(shareType){
   var s = s_gi(s_account);
   s.linkTrackVars = "prop1,prop29,eVar29,events";
   s.linkTrackEvents = "event6";
   s.prop29 = shareType;
   s.eVar29 = shareType;
   s.events = "event6";
   if(s.prop1) s.prop1 = s.prop1.toLowerCase();
   s.tl(this,'o','social network ' + shareType);
}

/* Email Share */
var scemailshare = false;
var scsocialshare = "";
if (scemailshare == true) {
	scsocialshare = "email";
}

/* Jump Page Function - event86 */
function scJump(){
	var s = s_gi(s_account);
	s.linkTrackVars = "eVar20,events";
	s.linkTrackEvents = "event86";
	s.eVar20 = "lexis jump page";
	s.events = "event86";
	s.tl(this,'e','jump to lexis');
}

/* Login Overlay Load Function - s.prop20, event85 */
function scoverlay(){
	var s = s_gi(s_account);
	s.linkTrackVars = "eVar20,prop20,events";
	s.linkTrackEvents = "event85";
	s.prop20 = "subscriber login";
	s.eVar20 = "subscriber login";
	s.events = "event85";
	s.tl(this,'o','load login overlay');
}

/* Login Overlay Close Function - event87 */
function scOverlayClose(){
	var s = s_gi(s_account);
	s.linkTrackVars = "eVar20,events";
	s.linkTrackEvents = "event87";
	s.eVar20 = "subscriber login";
	s.events = "event87";
	s.tl(this,'o','close login overlay');
}

/* Email Sent: Forms - event33 */
function scEmailForm(){
	var s = s_gi(s_account);
	s.linkTrackVars = "events";
	s.events = "event33";
	s.tl(this,'o','email form');
}
...
function s_doPlugins(s) {
	...
	/* SET CUSTOM PAGE VIEW EVENT */
		...
		s.setupLinkTrack("prop5,prop6,prop7","SC_LINKS");
		...
		/* Social Network exit links (Facebook, Twitter, Linkedin) -- prop6 */
			if(s.linkName in ary(['facebook', 'twitter','linkedin','fb']) || s.linkName == 'li' || s.linkName == 'in') {
				if (s.linkName == 'fb') {s.linkName = 'facebook';}
				if (s.linkName == 'in' || s.linkName == 'li') {s.linkName = 'linkedin';}
				s.prop6 = s.linkName;
			} else {
				//s.prop6 = 'exit: ' + s.prop6 + ' | ' + s.exitURL;
				s.prop6 = s.linkName;
			}
				//s.eVar35 = s.eo.href+'|'+s.eo.innerHTML.replace(/\s+/g,' ').replace(/<[^>]*>|\n|\t|\r/g,'');
				//s.eVar35 = s.eo.href;
				//s.eVar35 = s.eo.hostname + ' | ' + s.prop6;
				s.eVar35 = s.exitURL + ' | ' + s.prop6;
		...
		if(s.downloadURL)
		{
			...
			//s.setupLinkTrack("prop5,prop6,prop7,prop8","SC_LINKS");
			s.setupLinkTrack("prop5,prop6,prop7","SC_LINKS");
			s.linkTrackVars = s.apl(s.linkTrackVars,"prop43,eVar43,events",",",2);
			s.linkTrackEvents = s.apl(s.events,"event26",",",2);
			s.linkName = s.prop6.toLowerCase();
	...
	/* External Campaign Tracking */
		if(!s.campaign && s.getQueryParam('src'))
		{
			s.campaign = s.getQueryParam('src') + ':' + s.getQueryParam('cn') + ':' + s.getQueryParam('pt') + ':' + s.getQueryParam('et');
			if (s.getQueryParam('bu')) {
				s.campaign = s.getQueryParam('src') + ':' + s.getQueryParam('cn') + ':' + s.getQueryParam('bu') + ':' + s.getQueryParam('pt') + ':' + s.getQueryParam('et');
			}
			s.prop70 = s.getQueryParam('pt')  + ':' + s.getQueryParam('et');
			s.prop70=s.prop70.replace(/\+/g," ").replace(/\%20/g," ");
			s.eVar70 = "D=c70";

		}
		if(!s.campaign)
		{
			s.campaign=s.getQueryParam('cmp,rss,sharecode,affil');
	...
	/* Site Search */
		...
		if(s.prop9)
		{
			s.prop9 = s.getValOnce(s.prop9,'search_prop9',0);
			s.events = s.apl(s.events,'event2',',',2);
		}
		if(s.prop9)
		{
			s.eVar9='D=c9';
			s.prop39 = s.getPreviousValue(s.pageName,'qpv_pagename','');
			if(!s.prop39)
			{
				s.eVar39 = "D=r";
			}
	...
	/* Additional Internal Search Variables */
	if(s.prop53)
	{
		s.prop53 = s.getValOnce(s.prop53,'search_prop53',0);
		if(s.prop53)
		{
			s.eVar53 = "D=c53";
		}
	...
	if(s.prop54)
	{
		s.prop54 = s.getValOnce(s.prop54,'search_prop54',0);
		if(s.prop54)
		{
			s.eVar54 = "D=c54";
		}
	...
	if(s.prop55)
	{
		s.prop55 = s.getValOnce(s.prop55,'search_prop55',0);
		if(s.prop55)
		{
			s.eVar55 = "D=c55";
		}
	...
	/* TIME PARTING */
		...
		s.prop41=s.getTimeParting('h','-5',year).toLowerCase(); // Set hour
		s.eVar41="D=c41";
		s.prop42=s.getTimeParting('d','-5',year).toLowerCase(); // Set day
		s.eVar42="D=c42";

	/* Normalize props to lower case */
		if(s.prop1) s.prop1 = s.prop1.toLowerCase();
		if(s.prop2) s.prop2 = s.prop2.toLowerCase();
		if(s.prop3) s.prop3 = s.prop3.toLowerCase();
		if(s.prop4) s.prop4 = s.prop4.toLowerCase();
		if(s.prop33) s.prop33 = s.prop33.toLowerCase();
	...
	/* Copy props to eVars */
		...
		{
			s.eVar17 = s.prop17;
			s.prop17 = s.getValOnce(s.prop17,'s_prop17',0);
		}
	...
	var scproduct = "";
	if(s.server){
		scproduct = s.server.toUpperCase();
	}
	if(scproduct == "CLT"){ scproduct = "CT"; }
	if(scproduct == "NLJ"){ scproduct = "NL"; }
	if(scproduct == "LTN"){ scproduct = "IMLT"; }
	if(scproduct == "DRO"){ scproduct = "GA"; }
	if(scproduct == "FLM"){ scproduct = "DBR"; }
	if(scproduct == "NYLJ"){ scproduct = "NY"; }
	...
	if ((products=="") || (products==null)){
		...
		while (product < product_array.length){
			if(scproduct == 'CC' && ((product_array[product] == 'CC:LIMITED') || (product_array[product] == 'CC'))){
				userSubstatus = "freemium";
			}
			else if(scproduct == 'IMLT' && ((product_array[product] == 'IMLT:LIMITED') || (product_array[product] == 'IMLT'))){
				userSubstatus = "freemium";
			}
			else if(scproduct == 'IMLT' && ((product_array[product] == 'LT:LIMITED') || (product_array[product] == 'LT'))){
				userSubstatus = "freemium";
			}
			else if(product_array[product] == scproduct){
				userSubstatus = "premium";
			}
			else if(product_array[product] == scproduct+':LIMITED'){
				userSubstatus = "registered";
			}
			product+=1;
		}
	}
	...
	if(userSubstatus){
		s.prop22 = userSubstatus;
	} else if (!userSubstatus && s.prop21) {
		s.prop22 = "signed in";
	} else {
		s.prop22 = "anonymous";
	}
	...
	/* PLAC Product */
		...
		if(s.prop50)
		{
			...
			s.prop50 = s.getValOnce(s.prop50,'s_prop50',0);
		}
        if(s.eVar50)
        {
    ...
    if(s.getQueryParam('provider').toLowerCase() == 'linkedin')
    {
        s.referrer = s.getQueryParam('source');
    }
    s.plugins = "x";
}
